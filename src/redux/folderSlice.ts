...
const folderSlice = createSlice({
  ...
  reducers: {
    ...
    closeFolder: (state, action: PayloadAction<string>) => {
      state.openFolder = state.openFolder.filter(
    ...
    unMinimizeFolder: (state, action: PayloadAction<string>) => {
      state.minimizedFolders = state.minimizedFolders.filter(
...
export const { openFolders, closeFolder, minimizeFolder, unMinimizeFolder } =
  folderSlice.actions;
