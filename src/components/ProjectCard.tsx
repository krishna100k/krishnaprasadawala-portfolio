...
const ProjectCard: React.FC<{ projectData: MajorProject }> = ({projectData}) => {
  ...
  return (
    <div className={` w-full md:w-[48%] min-h-[25rem]  bg-cover bg-no-repeat border border-gray-800 relative overflow-hidden`} style={{ backgroundImage: `url(${projectData.image})` }}>
      <div onMouseEnter={mouseEnterHandler} onMouseLeave={mouseLeaveHandler} className="w-full h-14 hover:h-full overflow-y-auto overflow-x-hidden bg-black/55 backdrop-blur-lg absolute bottom-0 transition-all duration-500 border-t border-gray-800 cursor-pointer">
